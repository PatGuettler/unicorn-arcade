class_name RoomProceduralFurnitureBuilder
extends RefCounted
...
func build(parent: Node3D, requested_item_id: String, requested_category: String) -> void:
	item_id = requested_item_id
	category = requested_category
	_build(parent)


func _build(parent: Node3D) -> void:
	var palette := _palette()
	if item_id == "lamp":
		_build_lava_lamp(parent, palette)
	elif category == "rugs" or item_id == "rug":
		_build_rug(parent, palette)
	elif category == "nature" or item_id == "plant":
		_build_plant(parent, palette)
	elif category == "beds":
		_build_bed(parent, palette)
	elif category == "tables":
		_build_table(parent, palette)
	elif category == "lighting":
		_build_lamp(parent, palette)
	elif category == "pets":
		_build_pet(parent, palette)
	elif category == "toys":
		_build_toy(parent, palette)
	elif category == "electronics":
		_build_electronics(parent, palette)
	elif category == "seasonal":
		_build_seasonal(parent, palette)
	elif category == "kitchen":
		_build_kitchen(parent, palette)
	elif category == "wall":
		_build_wall_art(parent, palette)
	elif category == "luxury":
		_build_luxury(parent, palette)
	elif category == "unicorn":
		_build_unicorn_decor(parent, palette)
	else:
		_build_chair(parent, palette)
