...
func _run() -> void:
	...
	var companion_catalog_center: Vector2 = market.catalog_scroll.get_global_rect().get_center()
	var companion_press := InputEventScreenTouch.new()
	companion_press.pressed = true
	companion_press.position = companion_catalog_center
	market._input(companion_press)
	var companion_drag := InputEventScreenDrag.new()
	companion_drag.position = companion_catalog_center + Vector2(0, -180)
	companion_drag.relative = Vector2(0, -180)
	market._input(companion_drag)
	var companion_release := InputEventScreenTouch.new()
	companion_release.pressed = false
	companion_release.position = companion_drag.position
	market._input(companion_release)
	_check(market.catalog_scroll.scroll_vertical > companion_scroll_before and not market.catalog_dragging, "Companion catalog vertical drag uses the native scroll range and resets touch state")
	...
	market._catalog_suppress_until_ms = 0
	market.call("_companion_action", 1)
	...
	decor_press.position = decor_catalog_center
	market._input(decor_press)
	var decor_drag := InputEventScreenDrag.new()
	decor_drag.position = decor_catalog_center + Vector2(0, -220)
	decor_drag.relative = Vector2(0, -220)
	market._input(decor_drag)
	var decor_release := InputEventScreenTouch.new()
	decor_release.pressed = false
	decor_release.position = decor_drag.position
	market._input(decor_release)
	_check(market.catalog_scroll.scroll_vertical > decor_scroll_before and not market.catalog_dragging, "Decor catalog vertical drag uses the native scroll range and resets touch state")
	...
	var category_center: Vector2 = market.category_scroll.get_global_rect().get_center()
	var category_press := InputEventScreenTouch.new()
	category_press.pressed = true
	category_press.position = category_center
	market._input(category_press)
	var category_drag := InputEventScreenDrag.new()
	category_drag.position = category_center + Vector2(-180, 0)
	category_drag.relative = Vector2(-180, 0)
	market._input(category_drag)
	...
	market._input(category_release)
	_check(market.category_scroll.scroll_horizontal > scroll_before and not market.category_dragging, "Horizontal category drag scrolls the native strip and resets touch state on release")
	...
	market._category_suppress_until_ms = 0
	market.call("_guarded_category_action", "beds")
